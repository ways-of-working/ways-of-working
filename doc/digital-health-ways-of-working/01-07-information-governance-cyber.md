...
11. **Invest in detection, not just prevention.** Assume some attacks will get through your defences, and build the capability to see them: centralized logging, monitoring of key systems, and a security operations centre — in-house, shared across an ICS, or drawing on national services such as the NCSC and NHS England's cyber operations — so that suspicious activity is spotted and contained rather than discovered weeks later. Pair this with continuous vulnerability management and threat detection, so you patch the weaknesses attackers actually exploit. Detection buys you the time to contain an incident before it becomes an outage.
...
| Trust & transparency | Secondary uses opaque; opt-out patchy | Opt-out honoured; notices exist but unclear | Clear public information; opt-out reliably applied | Opt-out application and secondary-use disclosures tracked and audited against policy | Proactive engagement; commercial and AI uses disclosed openly |
