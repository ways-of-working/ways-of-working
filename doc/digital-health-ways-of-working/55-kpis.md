# Chapter 55 — Key Performance Indicators (KPIs)
...
In health and care, the numbers you watch shape the care people receive. A waiting-list KPI that counts "patients booked" but not "patients seen" can hide a cohort quietly deteriorating at home. A digital service dashboard that is green on uptime but silent on whether disabled users can complete a booking will let an access problem grow into a health inequality. What you measure signals what you value, and staff, suppliers, and algorithms all optimise towards the measured target — sometimes at the expense of the unmeasured thing that actually mattered.

The stakes are higher here than in most sectors because the failure modes are human. The Mid Staffordshire and other inquiries repeatedly found organisations that hit their headline financial and activity targets while care quality collapsed underneath. Metrics were reported upwards as green while patients came to harm — the pattern now widely called "watermelon reporting" (green on the outside, red on the inside), which Chapter 58 — Visibility for the CEO & Senior Leaders examines in depth. Good KPIs are one of your defences against that gap between the reported picture and the real one.

KPIs also spend public money and public trust. Every indicator you publish becomes part of how citizens, regulators, and commissioners judge the system. Over-claiming, cherry-picking, or gaming a target is not just poor management; it erodes the trust that digital health depends on to get people to share data and adopt new services. This chapter is the measurement partner to Chapter 54 — Objectives & Key Results (OKRs): where OKRs drive focused change, KPIs monitor the ongoing health of the services and systems you run.
...
- **Watermelon dashboards.** Aggregation that turns underlying red into surface green. Build honest reporting layers and audit the roll-up (Chapter 58).
...
|---|---|---|---|---|
| What is measured | Whatever the systems happen to log; mostly activity | A mix of activity and some outcomes, chosen ad hoc | A small balanced set of outcome, process, experience and balancing measures per service | Outcome- and equity-led KPIs, reviewed and retired on a cycle, tied to strategy and OKRs |
| How it is presented | Red/green tables, two-point comparisons | Some trend lines, mostly point-in-time | SPC run/control charts standard for operational data | SPC embedded in decision-making; noise vs signal understood by leaders |
| Equity | Aggregate figures only | Occasional ad hoc breakdowns | Routine segmentation of key KPIs by population | Segmentation plus deliberate effort to see the excluded and act on gaps |
| Gaming & data quality | Not considered; single targets under pressure | Awareness of gaming; data quality patchy and undisclosed | Balancing measures in place; data quality tracked and caveated | Metrics stress-tested against gaming; data quality measured as its own KPI |
| Reporting layers | One flattering view sent upward | Separate packs, inconsistent definitions | Layered reporting with agreed definitions and owners | Honest, audited roll-up from frontline to board; no watermelon gap |
...
- [ ] Outcome and experience measures are prioritised; raw activity sits in the supporting layer.
