...
8. **Use NICE's Evidence Standards Framework to right-size your evidence.** NICE's Evidence Standards Framework for Digital Health Technologies (ESF) tiers evidence expectations by the risk and function of the product, so a simple information tool is not held to the same bar as a treatment-guiding algorithm. Use it early to decide what evidence you genuinely need — and pair it with the Digital Technology Assessment Criteria (DTAC) for the safety, clinical-safety, and interoperability baseline (Chapters 6 and 25). Right-sizing prevents both under-evidenced hype and paralysing over-evidencing.
