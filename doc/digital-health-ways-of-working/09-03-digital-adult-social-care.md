...
6. **Design information sharing around the person, and make it two-way with health.** The value of a shared care record is realised at the interfaces: a care home that can see a resident's medications, or a hospital discharge team that can see a person's home-care package. Connect DSCRs to shared care records and use recognised standards so data flows both ways (see Chapter 4.3 — Interoperability & Data Standards). Sharing must be lawful, proportionate, and explainable to the person, with a clear basis under data protection law and honest conversations about consent.
