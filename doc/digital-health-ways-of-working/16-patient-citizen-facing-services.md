...
9. **Design for interoperability from the start.** A patient-facing service that cannot read from and write to the underlying records is a silo that creates double-keying and error. Use open standards so bookings, messages, and observations flow to and from the systems clinicians actually use, and so a person's data follows them across a fragmented care landscape (see Chapter 26 — Interoperability & Data Standards). The public experiences interoperability as "I don't have to repeat myself" — and its absence as danger and frustration.
