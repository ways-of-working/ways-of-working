...
The danger is using this tool for the wrong job. Project-led governance applied to an evolving digital service produces the "build it and abandon it" pathology described in Chapter 29 — Product-Led Work. This chapter explains where project-led work is the right instrument, and where its limits begin. It sits alongside Chapter 31 — Programme-Led Work, which coordinates multiple projects, and Chapter 32 — Enterprise Portfolio & Project Management (EPPM), which chooses between these tools.

## Core concepts

**Project.** A temporary organisation created to deliver one or more defined outputs to an agreed specification, within constraints of time, cost, and quality. The defining feature is that it is *temporary* — it exists to reach a defined end and then disband.
...
**[PRINCE2](https://en.wikipedia.org/wiki/PRINCE2) (PRojects IN Controlled Environments).** The UK government's structured project method, widely used across the NHS and public sector. It is built on principles (continued business justification, defined roles, manage by stages, manage by exception, focus on products, learn from experience, tailor to the environment) and organises work into controlled stages with a project board providing direction.
...
- **Project-managing an evolving service.** Applying fixed-scope, disband-at-the-end governance to a living digital product produces the abandonment pathology. Fix: use product-led work (Chapter 29) where the service is ongoing.
...
|---|---|---|---|---|
| Justification | Funding sought without options appraisal | Business case written once, then shelved | Living business case (Five Case) reviewed at each gate | Board routinely stops/re-plans projects that lose justification |
| Scope control | Scope vague; creep unmanaged | Scope defined but changes ad hoc | Scope baselined with formal change control | Change control is fast, transparent, and trusted |
| Governance | No clear board or roles | Board exists but gates are formalities | PRINCE2 roles clear; real stop/proceed gates | Manage-by-exception frees senior time for real decisions |
| Risk & control | Risks tracked informally or not at all | RAID log exists but stale | Live RAID; escalation by exception | RAID drives decisions; near-misses feed lessons |
| Closure & transition | Projects drift, never formally closed | Handover ad hoc | Deliberate close; service transition planned | Lessons systematically reused across projects |
