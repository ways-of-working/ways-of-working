...
**Product over project.** A project has a start, an end, and a hand-off; a [product](https://en.wikipedia.org/wiki/Product_management) is funded and stewarded for its whole life. The shift from project to product thinking (see Chapter 2 and Chapter 23 — Product-Led Work) changes funding, teams, and accountability, and it fits health services — which are long-lived and continuously evolving — far better than one-off projects do.

## Best practices

1. **Start from user needs, evidenced by research.** Before committing to a build, understand who the users are, what they are trying to do, and where the current service fails them. Fund a discovery (see Chapter 15 — Discovery Phases) and treat "we assumed" as a red flag. User needs, not internal preferences, are the anchor for every later decision.
...
6. **Design for interoperability and reuse.** Health and care data must flow safely between organisations; build to open standards ([HL7 FHIR](https://en.wikipedia.org/wiki/Fast_Healthcare_Interoperability_Resources), [SNOMED CT](https://en.wikipedia.org/wiki/SNOMED_CT)) and reuse platforms rather than re-inventing them (see Chapter 2's "thinnest viable platform" and Chapter 21 — Interoperability & Data Standards). A service that cannot share data is a silo waiting to become a risk.
...
- **Pilot purgatory.** Endless pilots that never scale because adoption, training, and business change were never resourced (see Chapter 38 — Change Management). Antidote: plan the path to scale before the pilot.
