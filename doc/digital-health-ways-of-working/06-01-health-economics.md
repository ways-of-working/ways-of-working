...
11. **Match the type of economic evaluation to the question, and always test affordability.** Cost-utility analysis and its ICER answer "is this good value per unit of health," but they are not the only tools. Use cost-benefit analysis, which values benefits in money, when you must weigh health returns against non-health ones; and run a budget impact analysis — the total cash cost of adopting the intervention across the system in a given year — alongside the ICER, because a service can be cost-effective per QALY yet unaffordable this year. Present value and affordability side by side, so a finance director sees both a defensible ICER and a number the budget can actually bear.

12. **Remember why the healthcare "market" fails, and evaluate as the substitute for it.** Value here cannot be left to ordinary market forces: patients rarely have the information to judge clinical value (information asymmetry between clinician and patient), benefits often spill onto uninvolved third parties (externalities such as the herd immunity one person's vaccination gives others), and the person consuming care is usually not the person paying for it. These are textbook market failures, and they are precisely why disciplined economic evaluation exists rather than price signals doing the work. Treat evaluation as the mechanism that stands in for a functioning market, not as optional paperwork.

13. **Use ready-made tools and borrow expertise for anything high-stakes.** For public-health and prevention decisions, start from the GOV.UK health-economics return-on-investment (ROI) tools and guidance for public health teams — which already cover areas such as falls prevention, mental health, and cardiovascular disease — rather than building a bespoke model from scratch. For contested or high-value cases, draw on health-economics expertise, whether an in-house analyst or an academic centre such as LSE Health or the Office of Health Economics. Reusing validated tools and specialist review makes your numbers faster to produce and far harder to dismiss.
