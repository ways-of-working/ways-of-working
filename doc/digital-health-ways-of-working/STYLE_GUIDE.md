...
7. `## Sector lenses` — the SAME topic applied in four different organizational contexts, as four `###` subsections in this order: `### Startup` (a small, fast, funding-constrained, usually pre-revenue digital-health company), `### Small business` (an established small or independent provider/SME — a GP practice, a care home, a single-site provider, or a small health-tech supplier past the startup stage), `### Enterprise` (a large, complex organization — an NHS trust, an ICS, or a big health-tech/pharma firm), `### Government` (a national or local public body — NHS England, DHSC, a devolved government, UKHSA, a local authority). Each is 3–6 sentences, concrete and specific, showing how the chapter's practice changes with organizational scale, funding model, risk appetite, and accountability.
8. `## Common failure modes` — a short list of anti-patterns and how to avoid them.
9. `## Maturity model` — a 5-level table (Initiate / Develop / Standardize / Manage / Orchestrate) with 3–5 rows of observable characteristics.
...
- Use **Oxford spelling** (`en-GB-oxendict`): British English with `-ize`/`-ization` (organize, prioritize, standardize, realize, optimize, recognize), but keep all other British forms — `-our` (behaviour), `-re` (centre), `-yse` (analyse, not analyze), doubled `l` (modelling), `licence`/`defence` nouns, and `-ae/-oe` (paediatric). Keep "program" only for a computer program; use "programme" for the delivery construct (programme management, MSP). Full rules and the URL/citation-protection mechanics are in [`spec/oxford-spelling.md`](spec/oxford-spelling.md). Be consistent within a chapter.
...
## Grounding
- Ground claims in recognized frameworks and public guidance. Real, checkable sources only — do not invent citations, URLs, statistics, or quotations. If unsure of a figure, describe the pattern qualitatively instead.
