...
3. **Never test with live patient data by default.** Provision test and staging environments with synthetic or robustly anonymised data, governed by the same information governance and DPIA discipline as production (see Chapter 1.7 — Information Governance, Data Protection & Cyber Security). If a specific test genuinely requires production-like data, treat it as a controlled exception with a documented lawful basis, minimisation, and audit — not as routine convenience. A test database is still a patient database.
