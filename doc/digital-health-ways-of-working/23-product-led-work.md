...
3. **Make outcome measures explicit and clinically meaningful.** Agree a small set of outcome metrics up front, ideally as [objectives and key results](https://en.wikipedia.org/wiki/OKR) (OKRs — see Chapter 34). Pair every efficiency metric with a safety and an equity metric, so a team is never rewarded for speed at the cost of harm or of widening inequality.
...
8. **Align teams through strategy, not micromanagement.** Autonomy without alignment produces local optimisation and chaos. A clear product vision and strategy — and, at scale, a portfolio and programme frame (Chapters 25 and 26) — is what lets you empower teams safely.
...
- **"Empowerment" without strategy.** Teams left to invent their own direction drift and duplicate. Fix: strong product vision and strategy, and portfolio-level alignment (Chapter 26).
- **Applying product-led to genuinely finite work.** A one-off data-centre migration or a hardware rollout does not need a perpetual team. Fix: use project-led work (Chapter 24) where scope really is fixed.
